...
fviz_nbclust(cw.agg) + theme_minimal() # kl and ch giving 0 as best k
```
...
```{r}
pm <- pam(tweet.num, k=2, metric = 'manhattan')
fviz_nbclust(tweet.num,pam, metric='manhattan')+ggtitle('PAM clustering for Civil War Data')
```

**Ans** Optimal number = 2


2.  Using the silhouette method, what is the optimal number of clusters?

```{r}
fviz_silhouette(pam(tweet.num, k=2, metric = 'manhattan'))+ggtitle('PAM clustering for Civil War Data')
```
...
for(ii in inds){
  ...
  tweets.nbc<-NbClust(tweet.num, method = ii)
  #####
...
```{r}
tweets.med <- kmedoids(tweet.num, 2)
fviz_nbclust(tweet.num, kmedoids, metric='manhattan')+ggtitle('KMedoids clustering for Civil War Data')
```
...
```{r}
fviz_silhouette(kmedoids(tweet.num, 2))+ggtitle('KMedoids clustering for Civil War Data')
```
